...
7. **Cron job model configuration** — If a cron job fails with `Cron job has no model configured` and `job.model=None`, the job needs an explicit model set. Fix with: `cronjob(action='update', job_id='<ID>', model={'model': '<MODEL_NAME>', 'provider': '<PROVIDER>'})`. To find the current model: `grep -o '"model"[^,]*' /data/.hermes/sessions/*.json | head -5` and `grep -o '"provider"[^,]*' /data/.hermes/sessions/*.json | head -5`. Do NOT create a new job — update the existing one.
