...
0. **Telegram channels break stories 30–90 minutes before Google News RSS** — Telegram channels (especially @VahidOnline, @alonews, @khabari_18) surface citizen reports, official confirmations, and breaking events well before Google News RSS indexes them. In this session, Qeshm Island explosions were reported by 10+ citizen eyewitnesss on @VahidOnline at 21:40 IRST — Google News RSS didn't carry the story until ~5 hours later. For time-critical monitoring: **scan Telegram FIRST → then Google News RSS for broader coverage and cross-validation.** Telegram is the fastest-sensor layer; RSS is the validation layer. The structured extraction pipeline (Phase 0) should run before Phase 1 (broad discovery), not in parallel.
...
27. **Al Jazeera liveblog URLs return 404** — AJ liveblog URLs (e.g. `/news/2026/8/7/...liveblog-...`) now return 404 pages via curl as of Aug 7, 2026. The liveblog format has been retired or restructured. Regular AJ article pages (`/news/YYYY/MM/DD/...`) still work fine. For live coverage, use Google News RSS with `site:aljazeera.com` instead of scraping liveblog URLs directly.

28. **Brave Search intermittently redirects to Tor in server environments** — On some server/subagent environments, Brave Search HTML requests may return a Tor Project page instead of search results. This appears to be an IP-level routing issue. If Brave fails, fall back to Google News RSS which is more reliable from server IPs. Do NOT retry Brave with the same query — it will keep failing for the same session.
