...
This skill produces open-source intelligence (OSINT) situational awareness reports focused on Iran, the Persian Gulf, the Strait of Hormuz, Kharg Island, Iran's southern coasts, and the broader regional security environment — including US forces, Israel, and Gulf states. Reports are compiled exclusively from publicly available sources and are designed for civilian awareness, not operational planning.

**Language:** All output is in Persian (Farsi) with exact Gregorian and Jalali dates and timestamps.

**No cron or scheduled execution** is configured by this skill. It runs on-demand when invoked.
...
2. **وضعیت کلی** — Overall status: 🟢 سبز / 🟡 زرد / 🟠 نارنجی / 🔴 قرمز
3. **رویدادهای تأییدشده** — Confirmed events with sources
4. **ادعاهای در حال بررسی** — Unverified claims under review
5. **وضعیت خارک و سواحل جنوبی** — Kharg Island & southern coast status
6. **وضعیت تنگه هرمز و کشتیرانی** — Strait of Hormuz & shipping status
7. **تحرکات سیاسی و نظامی منطقه** — Regional political & military movements
8. **شاخصهای هشدار زودهنگام** — Early warning indicators
9. **سناریوهای 24 تا 72 ساعت آینده** — 24–72 hour scenarios (labeled «ارزیابی»)
10. **تغییردهندههای ارزیابی** — What could change this assessment
11. **اقدامات عمومی پیشنهادی** — Civilian recommendations
12. **سیگنالهای سریع شبکههای اجتماعی** — Social media fast-sensor signals
13. **فهرست منابع و سطح اطمینان** — Source list with confidence levels
...
```
RuntimeError: Cron job 'gulf-kharg-watch-daily' has no model configured (job.model=None, HERMES_MODEL='', config.yaml model.default missing or empty). Set a per-job model via `cronjob action=update job_id=d4a614a366a1 model=<name>` or set a default with `hermes model <name>`.
```
