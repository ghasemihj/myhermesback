# Cron Job: crypto-portfolio-daily (FAILED)

**Job ID:** ba737d8a9800
**Run Time:** 2026-07-31 19:12:05
**Schedule:** 30 7 * * *
...
```
RuntimeError: Cron job 'crypto-portfolio-daily' has no model configured (job.model=None, HERMES_MODEL='', config.yaml model.default missing or empty). Set a per-job model via `cronjob action=update job_id=ba737d8a9800 model=<name>` or set a default with `hermes model <name>`.
```
