...
Daily Gulf-Kharg Watch situational awareness report. Load skill gulf-kharg-watch and generate a 24-hour rolling report.

CRITICAL DELIVERY RULES:
1. NEVER send the full report as ONE message. Telegram has a message length limit.
2. Split the report into MULTIPLE messages. Each message must be max 3500 characters.
3. Send messages in this order:
   - Message 1: Header + Executive Summary + Fast Alerts table
   - Message 2: Overall Status + Confirmed Events
   - Message 3: Under Review Claims + Iran Situation + Regional Situation
   - Message 4: Hormuz and Shipping + Early Warning Indicators
   - Message 5: Scenarios + Assessment Changers + Recommendations
   - Message 6: Social Media Signals
   - Message 7: Kharg Impact + Source List
4. Between messages, briefly state what section is coming next.
5. After ALL messages are sent, save the COMPLETE report as a single file to /data/.hermes/reports/gulf-kharg-watch/ using the naming convention report-YYYY-MM-DDTHH-MM.md and update latest.md symlink.
6. NEVER truncate or skip any section. Every section from the template must appear in the delivered messages.

Save the full report to file FIRST, then deliver the split messages to Telegram. This way even if delivery fails, the report is preserved on disk.
